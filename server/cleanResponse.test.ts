...
describe("Bereinigung interner Markierungen", () => {
  it("entfernt Markierungen samt vorangehendem Leerzeichen", () => {
    const roh =
      "Levin, Ihr Sohn, geboren am 15. Juni 2019 [person]. Ich erlaube mir den Hinweis.";
    expect(removeInternalTags(roh)).toBe(
      "Levin, Ihr Sohn, geboren am 15. Juni 2019. Ich erlaube mir den Hinweis."
    );
  });

  it("entfernt mehrere verschiedene Markierungen in einem Text", () => {
    const roh =
      "Schulstart am 17. August 2026 [context]. Lina, geboren am 16. August 2024 [person]; Geburtstag steht an [context].";
  ...
  it("arbeitet unabhängig von der Gross-/Kleinschreibung", () => {
    expect(removeInternalTags("Notiz [Person] und [CONTEXT] weg")).toBe(
      "Notiz und weg"
    );
  });
...
describe("Gesprochene Fassung", () => {
  it("spricht interne Markierungen nicht mit", () => {
    const roh =
      "Levin, geboren am 15. Juni 2019 [person]. Ein bedeutender Meilenstein.";
    const gesprochen = stripForSpeech(roh);
    expect(gesprochen).not.toContain("[person]");
    expect(gesprochen).toBe(
      "Levin, geboren am 15. Juni 2019. Ein bedeutender Meilenstein."
    );
  });
  ...
  it("spricht normale Antworten vollständig aus", () => {
    ...
    const antwort =
      "Meinen Aufzeichnungen zufolge, Sir, sind Sie stolzer Vater von zwei Kindern. " +
  ...
  it("bildet bei sehr langen Texten eine Kurzfassung statt hart abzuschneiden", () => {
    const langerText =
      "Dies ist ein vollständiger Satz mit ausreichender Länge. ".repeat(40) +
    ...
    expect(
      spoken.trim().endsWith("Soll ich eine Erinnerung einrichten, Sir?")
    ).toBe(true);
  });
  ...
  it("spricht das Schritt-Protokoll nicht mit", () => {
    const mitProtokoll =
      "Die Rechnung ist bezahlt.⟦schritte⟧mark_invoice_paid | list_invoices";

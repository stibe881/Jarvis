...
describe("Geräte-Befehle (iOS-Kurzbefehle)", () => {
  it("beschreibt eine WhatsApp-Nachricht lesbar", () => {
    const text = describeDeviceCommand("whatsapp", { recipient: "Bine", message: "Ich komme später" });
    expect(text).toContain("Bine");
    expect(text).toContain("Ich komme später");
  });

  it("beschreibt einen Wecker mit Uhrzeit", () => {
    expect(describeDeviceCommand("alarm", { time: "06:30", label: "Aufstehen" })).toContain("06:30");
  });
...
describe("Spotify-Aktionsblock-Parser", () => {
  ...
  it("erkennt mehrere Blöcke", () => {
    const answer = '<spotify_action>{"action":"pause"}</spotify_action><spotify_action>{"action":"volume","level":30}</spotify_action>';
  ...
  it("lässt Antworten ohne Block unverändert", () => {
    const { found, clean } = parseSpotifyBlocks("Ich habe keine Musik gestartet.");

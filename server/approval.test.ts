...
describe("Kritische Aktionen erkennen", () => {
  ...
  it("beschreibt kritische Aktionen verständlich mit Daten", () => {
    expect(
      describeCritical("app_action", "mark_invoice_paid", { id: "R-2026-014" })
    ).toContain("R-2026-014");
    expect(
      describeCritical("device_action", "whatsapp", {
        recipient: "Bine",
        message: "Komme später",
      })
    ).toContain("Bine");
    expect(
      describeCritical("calendar_action", "delete_event", {
        summary: "Zahnarzt",
      })
    ).toContain("Zahnarzt");
  });
...
describe("Bestätigungspflicht in der Agenten-Schleife", () => {
  it("führt eine kritische Aktion NICHT aus, sondern merkt sie vor", async () => {
    ...
    const result = await runAgentLoop({
      firstResponse:
        '<app_action>{"action":"mark_invoice_paid","id":"R-2026-014"}</app_action>',
      messages: [],
    ...
    const result = await runAgentLoop({
      ...
      runAction: async parsed => {
        ...
        return {
          kind: parsed.tag,
          action: "mark_invoice_paid",
          result: "Rechnung als bezahlt markiert",
  ...
  it("führt harmlose Aktionen ohne Freigabe direkt aus", async () => {
    const result = await runAgentLoop({
      firstResponse:
        '<tasks_action>{"action":"create","title":"Nachfassen"}</tasks_action>',
      messages: [],
  ...
  it("trennt harmlose und kritische Aktionen in derselben Runde", async () => {
    const result = await runAgentLoop({
      firstResponse:
        '<app_action>{"action":"list_invoices"}</app_action><app_action>{"action":"mark_invoice_paid","id":"R-1"}</app_action>',
      messages: [],
      runAction: fakeRunner({
        list_invoices: "5 offen",
        mark_invoice_paid: "bezahlt",
      }),
      callModel: async () =>
        "Fünf Rechnungen offen. Soll ich R-1 als bezahlt markieren?",
    });
...
describe("Freigabe im Gespräch erkennen", () => {
  ...
  it("wertet eine neue inhaltliche Anfrage nicht als Freigabe", () => {
    expect(isApproval("Zeig mir die offenen Tickets von Muster AG bitte")).toBe(
      false
    );
...
describe("Ablehnung erkennen", () => {
  ...
  it("wertet eine neue Anfrage nicht als Ablehnung", () => {
    expect(
      isRejection("Zeig mir bitte alle offenen Rechnungen von Muster AG")
    ).toBe(false);
  });
});
...
describe("Freigabe-Ablauf im Gespräch", () => {
  ...
  async function handleTurn(
    message: string,
    executor: (p: PendingAction) => Promise<string>
  ) {
    ...
    if (rejected) {
      clearPending(CONV);
      return { executed: [] as string[], state: "rejected" as const };
    }
    if (approved) {
    ...
    return {
      executed: [] as string[],
      state: wasPending ? ("still-pending" as const) : ("none" as const),
    };
  }
  ...
  it("führt eine kritische Aktion erst nach klarem Ja aus", async () => {
    ...
    const loop = await runAgentLoop({
      ...
      messages: [],
      runAction: async () => {
        throw new Error("darf nicht ausgeführt werden");
      },
    ...
    const mid = await handleTurn(
      "Wie hoch war der Betrag nochmal?",
      async () => "sollte nicht laufen"
    );

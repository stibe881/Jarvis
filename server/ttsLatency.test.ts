...
const SATZ =
  "Guten Morgen Stefan. Du hast heute drei Termine und zwei offene Aufgaben.";
...
describe("Antwortzeit der Sprachausgabe", () => {
  it("der Streaming-Endpunkt liefert bereits ein erstes Audiopaket, bevor alles fertig ist", async () => {
    ...
    try {
      ...
      if (istNetzfehler(e)) {
        console.warn("[Stream] übersprungen – Netzwerkproblem der Umgebung");
        return;
      }
      throw e;
    }
    if (!resp.ok) {
      console.warn(
        `[Stream] übersprungen – Guthaben aufgebraucht (HTTP ${resp.status})`
      );
      return;
    }
    ...
    console.log(
      `[Stream] erstes Paket nach ${ttfb} ms, vollständig nach ${complete} ms (${total} Bytes)`
    );
  ...
  it("das schnelle Modell liefert gültiges Audio", async () => {
    const flash = await mitWiederholung(() =>
      messeLatenz("eleven_flash_v2_5", "4", "mp3_22050_32")
    );
    console.log(
      `[Flash] ${flash.ms} ms, ${flash.bytes} Bytes, Status ${flash.status}`
    );
    if (!flash.ok) {
      // 401 mit quota_exceeded bedeutet: Monatsguthaben aufgebraucht – kein Codefehler
      console.warn(
        `[Flash] übersprungen – Guthaben aufgebraucht (HTTP ${flash.status})`
      );
      return;
    }
  ...
  it("liefert bei kleinem Ausgabeformat weniger Daten als das hochauflösende", async () => {
    ...
    const klein = await mitWiederholung(() =>
      messeLatenz("eleven_flash_v2_5", "4", "mp3_22050_32")
    );
    const gross = await mitWiederholung(() =>
      messeLatenz("eleven_multilingual_v2", "0", "mp3_44100_128")
    );
    console.log(
      `[Datenmenge] klein ${klein.bytes} B (${klein.ms} ms) vs gross ${gross.bytes} B (${gross.ms} ms)`
    );
    if (!klein.ok || !gross.ok) {
      console.warn(
        `[Datenmenge] übersprungen – Guthaben aufgebraucht (HTTP ${klein.status}/${gross.status})`
      );
      return;
    }

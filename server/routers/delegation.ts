...
import {
  createDelegation, getDelegationsByUser, updateDelegation, deleteDelegation,
  getUserProfile,
} from "../db";
...
Gib nur den E-Mail-Text zurück, ohne weitere Erklärungen.`;
  ...
  regenerateEmail: protectedProcedure
    .input(z.object({ id: z.number(), tone: z.enum(["freundlich", "sachlich", "dringend"]).default("freundlich") }))
    .mutation(async ({ ctx, input }) => {
      const all = await getDelegationsByUser(ctx.user.id);
      const d = all.find(x => x.id === input.id);
      if (!d) throw new TRPCError({ code: "NOT_FOUND", message: "Delegation nicht gefunden." });
      const profile = await getUserProfile(ctx.user.id);
      const senderName = profile?.displayName ?? ctx.user.name ?? "Stefan Gross";

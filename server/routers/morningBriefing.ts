...
async function getTodayEvents(userId: number): Promise<string[]> {
  try {
    ...
    const todayStart = new Date(now.toLocaleDateString("en-CA", { timeZone: tz }) + "T00:00:00+02:00");
    const todayEnd = new Date(now.toLocaleDateString("en-CA", { timeZone: tz }) + "T23:59:59+02:00");
    ...
    return data.items.map(ev => {
      ...
      const timeStr = ev.start?.dateTime
        ...
        : "Ganztägig";
      const inviteStr = ev.organizer && !ev.organizer.self
    ...
    const openTasks = await db.select().from(tasks).where(
      and(
        ...
        or(
          ...
          lte(tasks.dueDate, todayEnd.getTime())
        )
      )
    ).limit(10);
...
export async function handleMorningBriefing(req: Request, res: Response) {
  try {
    ...
    console.log(`[MorningBriefing] Briefing gesendet für ${ownerName}: ${events.length} Termine, ${todayTasks.length} Aufgaben`);

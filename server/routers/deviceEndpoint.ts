...
import { findWebhookKey, touchWebhookKey, claimPendingDeviceCommands, markDeviceCommandDone } from "../db";
...
export async function handleDeviceCommandsFetch(req: Request, res: Response) {
  try {
    ...
    if (!keyRow) return res.status(401).json({ error: "Ungültiger API-Schlüssel" });

    const rows = await claimPendingDeviceCommands(keyRow.userId, 10);
    const commands = rows.map((r) => {
      let params: Record<string, unknown> = {};
      try { params = JSON.parse(r.payload) as Record<string, unknown>; } catch { /* leer */ }
...
export async function handleDeviceCommandDone(req: Request, res: Response) {
  try {
    ...
    if (!id || Number.isNaN(id)) return res.status(400).json({ error: "id fehlt" });

    await markDeviceCommandDone(id, keyRow.userId, body.status === "failed" ? "failed" : "done");

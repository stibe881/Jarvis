...
const addressOptions = [
  {
    value: "sir",
    label: "Sir / Ma'am (formell)",
    desc: "Jarvis spricht dich förmlich an",
  },
  {
    value: "du",
    label: "Du (persönlich)",
    desc: "Jarvis spricht dich persönlich an",
  },
...
export default function JarvisProfile() {
  ...
  const updateMutation = trpc.profile.update.useMutation({
    onSuccess: () =>
      toast.success("Profil gespeichert – Jarvis kennt dich jetzt besser!"),
  ...
  const previewVoice = (voiceId: string, name: string) => {
    ...
    ttsMutation.mutate(
      {
        text: `Hallo, ich bin ${name}. Wie kann ich Ihnen helfen, Sir?`,
        voiceId,
        shorten: false,
      },
      {
  ...
  return (
    <div className="flex-1 overflow-auto px-4 md:px-8 py-6 max-w-2xl mx-auto w-full">
      ...
      <div className="flex items-center gap-3 mb-8">
        ...
        <div>
          <h1 className="font-jarvis text-xl font-bold text-primary">
            PROFIL & EINSTELLUNGEN
          </h1>
          <p className="text-sm text-muted-foreground">
            Hilf Jarvis dich besser kennenzulernen
          </p>
        </div>
      </div>
      ...
      <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground flex items-center gap-1">
                <Briefcase size={12} /> Beruf
              </Label>
              <Input
                value={form.occupation}
                onChange={set("occupation")}
                placeholder="z.B. Unternehmer, Entwickler..."
                className="bg-input border-border"
              />
            </div>
        ...
        <section className="jarvis-card rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              value={form.jarvisName}
              onChange={set("jarvisName")}
              placeholder="Jarvis"
              className="bg-input border-border"
            />
            <p className="text-xs text-muted-foreground">
              So nennt sich dein Assistent selbst
            </p>
          </div>
          ...
          <div className="space-y-2">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <MessageSquare size={12} /> Anredeform
            </Label>
              ...
              {addressOptions.map(opt => (
                <button
                  ...
                  onClick={() =>
                    setForm(f => ({
                      ...f,
                      addressForm: opt.value as "sir" | "du" | "name",
                    }))
                  }
                  className={cn(
              ...
              {languageOptions.map(opt => (
                <button
                  ...
                  onClick={() =>
                    setForm(f => ({
                      ...f,
                      language: opt.value as "de" | "en" | "auto",
                    }))
                  }
                  className={cn(
        ...
        <section className="jarvis-card rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Textarea
              ...
              placeholder="z.B. Technologie, Sport, Familie, Kochen..."
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground flex items-center gap-1">
              <Briefcase size={12} /> Beruflicher Kontext
            </Label>
            <Textarea
              value={form.workContext}
              onChange={set("workContext")}
              placeholder="z.B. Ich führe ein Unternehmen im Bereich... Meine Hauptaufgaben sind..."
              className="bg-input border-border resize-none"
              rows={3}
            />
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">
              Weitere persönliche Notizen
            </Label>
            <Textarea
              value={form.personalNotes}
              onChange={set("personalNotes")}
              placeholder="z.B. Ich habe zwei Kinder, lebe in der Schweiz, spreche Deutsch und Englisch..."
              className="bg-input border-border resize-none"
              rows={3}
            />
          </div>
        ...
        <section className="jarvis-card rounded-xl p-5 space-y-4">
          ...
          <div className="space-y-1.5">
            <Label className="text-xs text-muted-foreground">
              Wie soll sich Jarvis verhalten?
            </Label>
            <Textarea
            ...
            />
            <p className="text-xs text-muted-foreground">
              Beschreibe in eigenen Worten wie Jarvis sich verhalten soll
            </p>
          </div>
        </section>
        ...
        <section className="jarvis-card rounded-xl p-5 space-y-4">
          ...
          <p className="text-xs text-muted-foreground">
            Wähle die Stimme mit der Jarvis mit dir spricht. Klicke auf ▶ um
            eine Vorschau zu hören.
          </p>
            ...
            <div className="space-y-1.5 rounded-lg border border-border p-3">
              <div className="flex items-center justify-between text-xs">
                <span className="text-muted-foreground">
                  Sprachausgabe-Budget ({ttsUsage.yearMonth})
                </span>
                <span
                  className={cn(
                ...
                >
                  {ttsUsage.charsUsed.toLocaleString("de-CH")} /{" "}
                  {ttsUsage.limit.toLocaleString("de-CH")} Zeichen
                </span>
              </div>
              ...
              <p className="text-[10px] text-muted-foreground">
                Jarvis spricht eine Kurzfassung (max. 450 Zeichen), damit das
                Monatsbudget für viele Antworten reicht. Der vollständige Text
                bleibt im Chat sichtbar.
              </p>
            </div>
          )}
          {!voicesData || voicesData.length === 0 ? (
            <div className="flex items-center gap-2 text-muted-foreground text-xs">
              <Loader2 size={12} className="animate-spin" /> Stimmen werden
              geladen...
            </div>
          ) : (
              ...
              {voicesData.map(
                (v: {
                  ...
                  <div
                    ...
                    onClick={() =>
                      setForm(f => ({ ...f, elevenLabsVoiceId: v.id }))
                    }
                  >

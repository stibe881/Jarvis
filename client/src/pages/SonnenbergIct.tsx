...
function exportDocPdf(title: string, content: string, footer: string) {
  ...
  doc.text(new Date().toLocaleDateString("de-CH"), pageW - margin, 24, {
    align: "right",
  });
  y = 52;
  ...
  doc.save(
    `${title.replace(/\s+/g, "_")}_${new Date().toLocaleDateString("de-CH").replace(/\./g, "-")}.pdf`
  );
}
    ...
    <div className="space-y-4">
      <div className="jarvis-card rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            >
              ...
              <SelectContent>
                ...
                <SelectItem value="security_concept">
                  Sicherheitskonzept
                </SelectItem>
                <SelectItem value="migration_plan">Migrationsplan</SelectItem>
                <SelectItem value="infrastructure_plan">
          ...
          <div>
            ...
            <Input
              value={form.topic}
              onChange={set("topic")}
              placeholder="z.B. WLAN-Erneuerung, Microsoft 365 Migration..."
          ...
          <div>
            ...
            <Input
              value={form.scope}
              onChange={set("scope")}
              placeholder="z.B. alle Standorte, nur Verwaltung..."
              className="bg-input border-border mt-1"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="z.B. Veraltete Infrastruktur, Sicherheitslücken..."
              className="bg-input border-border mt-1"
            />
          </div>
        </div>
        <Button
          ...
          disabled={!form.topic || mutation.isPending}
        ...
        >
          {mutation.isPending ? (
            ...
            <>
              <FileText size={12} /> Konzept generieren
            </>
          )}
        </Button>
      </div>
        ...
        <ResultBox
          ...
          onPdf={() =>
            exportDocPdf(
              ...
              "SONNENBERG Baar · ICT-Abteilung · stefan.gross@sonnenberg-baar.ch"
            )
          }
        />
      )}
  ...
  return (
    <div className="space-y-4">
      <div className="jarvis-card rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div className="md:col-span-2">
            ...
            <Input
              value={form.item}
              onChange={set("item")}
              placeholder="z.B. 10x Laptop Dell Latitude 5540, Firewall Fortinet..."
              className="bg-input border-border mt-1"
            />
          </div>
          <div>
          ...
          <div>
            ...
            <Select
              value={form.urgency}
              onValueChange={v =>
                setForm(f => ({ ...f, urgency: v as typeof form.urgency }))
              }
            >
              <SelectTrigger className="bg-input border-border mt-1">
                <SelectValue />
        ...
        <div>
          ...
          <Textarea
            ...
            rows={2}
            placeholder="Warum wird diese Beschaffung benötigt?"
        ...
        <Button
          ...
          disabled={!form.item || mutation.isPending}
          className="bg-blue-600 hover:bg-blue-700 text-white gap-1"
        >
          {mutation.isPending ? (
            <>
            ...
            <>
              <ClipboardList size={12} /> Antrag generieren
            </>
          )}
        </Button>
      </div>
      {mutation.data && (
        <ResultBox
          content={mutation.data.content}
          pdfTitle={`Beschaffungsantrag_${form.item}`}
          onPdf={() =>
            exportDocPdf(
              `Beschaffungsantrag: ${form.item}`,
              mutation.data!.content,
              "SONNENBERG Baar · ICT-Abteilung · stefan.gross@sonnenberg-baar.ch"
            )
          }
  ...
  return (
    <div className="space-y-4">
      <div className="jarvis-card rounded-xl p-4 space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            ...
            <Input
              ...
              placeholder="z.B. ICT-Abteilungssitzung August 2026"
              className="bg-input border-border mt-1"
            />
          </div>
          <div>
            ...
            <Input
              ...
              placeholder="z.B. Stefan Gross, Max Muster, ..."
              className="bg-input border-border mt-1"
            />
          </div>
        </div>
        <div>
          ...
          <Textarea
            ...
            rows={6}
            placeholder="Tippe oder füge deine Sitzungsnotizen hier ein – Jarvis strukturiert sie professionell..."
            className="bg-input border-border mt-1 resize-none"
          />
        </div>
        <Button
          onClick={() => mutation.mutate(form)}
          disabled={!form.rawNotes || mutation.isPending}
          className="bg-blue-600 hover:bg-blue-700 text-white gap-1"
        >
          {mutation.isPending ? (
            <>
              <Loader2 size={12} className="animate-spin" /> Strukturiere...
            </>
          ) : (
            <>
              <ClipboardList size={12} /> Protokoll erstellen
            </>
          )}
        </Button>
      </div>
      {mutation.data && (
        <ResultBox
          content={mutation.data.content}
          pdfTitle={form.meetingTitle || "Sitzungsprotokoll"}
          onPdf={() =>
            exportDocPdf(
              form.meetingTitle || "Sitzungsprotokoll",
              mutation.data!.content,
              "SONNENBERG Baar · ICT-Abteilung · stefan.gross@sonnenberg-baar.ch"
            )
          }
  ...
  return (
    <div className="space-y-4">
      <div className="jarvis-card rounded-xl p-4 space-y-3">
        ...
        <div>
          <Label className="text-xs text-muted-foreground">
            Erledigte Aufgaben / Projekte
          </Label>
          <Textarea
            value={form.completedItems}
            onChange={set("completedItems")}
            rows={2}
        ...
        <div>
          ...
          <Textarea
            value={form.ongoingItems}
            onChange={set("ongoingItems")}
            rows={2}
            placeholder="Was ist in Bearbeitung?"
            className="bg-input border-border mt-1 resize-none"
          />
        </div>
        <div>
          ...
          <Textarea
            value={form.plannedItems}
            onChange={set("plannedItems")}
            rows={2}
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={set("issues")}
            rows={2}
            placeholder="Offene Probleme, Risiken..."
            className="bg-input border-border mt-1 resize-none"
          />
        </div>
        <Button
        ...
        >
          {mutation.isPending ? (
            ...
            <>
              <BarChart3 size={12} /> Statusbericht generieren
            </>
          )}
        </Button>
      </div>
      {mutation.data && (
        <ResultBox
          content={mutation.data.content}
          pdfTitle={`IT-Statusbericht_${form.period || "aktuell"}`}
          onPdf={() =>
            exportDocPdf(
              `IT-Statusbericht ${form.period || ""}`,
              mutation.data!.content,
              "SONNENBERG Baar · ICT-Abteilung · stefan.gross@sonnenberg-baar.ch"
            )
          }
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
          ...
          <div>
            ...
            <Select
              value={form.audience}
              onValueChange={v =>
                setForm(f => ({ ...f, audience: v as typeof form.audience }))
              }
            >
              <SelectTrigger className="bg-input border-border mt-1">
                <SelectValue />
          ...
          <div className="md:col-span-2">
            ...
            <Input
              value={form.topic}
              onChange={set("topic")}
              placeholder="z.B. Passwort-Richtlinie, Systemwartung am Wochenende..."
              className="bg-input border-border mt-1"
            />
          </div>
        </div>
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="px-4 md:px-6 py-4 border-b border-border flex-shrink-0">
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <h1 className="font-jarvis text-lg font-bold text-blue-400">
              SONNENBERG BAAR
            </h1>
            <p className="text-xs text-muted-foreground">
              ICT-Assistent · Leiter ICT
            </p>
          </div>
        </div>
        {/* Tabs */}

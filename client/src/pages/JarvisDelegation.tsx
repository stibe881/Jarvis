...
export default function JarvisDelegation() {
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Aufgaben zuweisen – Jarvis schreibt die E-Mail dazu
          </p>
        </div>
      ...
      {isLoading && (
        <p className="text-sm text-muted-foreground">Lade Delegationen…</p>
      )}
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  An {d.assigneeName}
                  {d.assigneeEmail ? ` (${d.assigneeEmail})` : ""}
                  {d.dueDate
                    ? ` · Frist ${new Date(d.dueDate).toLocaleDateString("de-CH")}`
                    : ""}
                </p>
              </div>
              ...
              <div className="rounded-md border border-border bg-muted/30 p-3 space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  <Button
                    ...
                    onClick={() => {
                      navigator.clipboard.writeText(d.emailDraft ?? "");
                  ...
                  <a href={mailtoLink(d.assigneeEmail, d.emailDraft)}>
                    <Button
                      ...
                      className="gap-1.5 text-xs"
                      onClick={() =>
                        updateMutation.mutate({ id: d.id, markEmailSent: true })
                      }
                    >
                  ...
                  <Button
                    ...
                    className="gap-1.5 text-xs"
                    onClick={() =>
                      regenMutation.mutate({ id: d.id, tone: "dringend" })
                    }
                  ...
                  {d.emailSentAt && (
                    <span className="text-[10px] text-primary flex items-center gap-1">
                      <CheckCircle2 size={12} /> versendet am{" "}
                      {new Date(d.emailSentAt).toLocaleDateString("de-CH")}
                    </span>
                  )}
                </div>
              </div>
      ...
      <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
        <DialogContent className="max-w-lg">
          <DialogHeader>
            <DialogTitle className="font-jarvis">
              Aufgabe delegieren
            </DialogTitle>
          </DialogHeader>
          <div className="space-y-3">
            <Input
              placeholder="Name der Person, z.B. Bine"
              value={name}
            ...
            />
            <Textarea
              placeholder="Details und Kontext (optional)"
              value={details}
